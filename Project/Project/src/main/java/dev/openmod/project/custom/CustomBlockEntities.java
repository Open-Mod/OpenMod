...
public class ModBlockEntities {
    ...
    public static void init() {
        for(RegistryObject<Block> blockEntry : BlockInit.BLOCKS.getEntries()) {
            BLOCK_ENTITIES_MAP.put(blockEntry.getKey().location().getPath(),  BLOCK_ENTITIES.register(blockEntry.getKey().location().getPath() + "_entity", () -> BlockEntityType.Builder.of((pos, state) -> new CustomBlock(blockEntry.getKey().location().getPath(), pos, state),
