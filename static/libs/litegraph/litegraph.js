...
(function (global) {
  ...
  var LiteGraph = (global.LiteGraph = {
    ...
    registerNodeType: function (type, base_class) {
      ...
      base_class.category ??= [];
      base_class.category.push(type.substr(0, pos));
    ...
    getNodeTypesInCategory: function (category, filter) {
      ...
      for (var i in this.registered_node_types) {
        ...
        if (r.includes(type)) continue;
        if (category == "") {
          if (type.category == null) {
            r.push(type);
          }
        } else if (type.category.includes(category)) {
          r.push(type);
        }
      }
    ...
    getNodeTypesCategories: function (filter) {
      ...
      for (var i in this.registered_node_types) {
        var type = this.registered_node_types[i];
        if (type.category && !type.skip_list && type.showInContext != false) {
          if (type.filter != filter) continue;
          type.category.forEach((c) => {
            categories[c] = 1;
          });
        }
      }
...
*/
  ...
  LGraphNode.prototype.clone = function () {
    ...
    console.log(this.constructor.showInContext);
  ...
  LGraphNode.prototype.computeSize = function (out) {
    ...
    function compute_text_size(text) {
      ...
      return font_size * text.length * 0.6 + 12;
    }
...
*/
  ...
  LGraphCanvas.prototype.pasteFromClipboard = function () {
    ...
    for (var i = 0; i < clipboard_info.nodes.length; ++i) {
      ...
      if (
        node &&
        LiteGraph.registered_node_types[node_data.type].showInContext
      ) {
  ...
  LGraphCanvas.prototype.resize = function (width, height) {
    if (!width && !height) {
      var parent = this.canvas.parentElement.getBoundingClientRect();
      width = parent.width;
  ...
  LGraphCanvas.onShowPropertyEditor = function (
    ...
    if (rect) {
      offsetx -= rect.left;
      offsety -=
        rect.top - document.getElementById("editor").offsetTop - rect.height;
    }
    if (event) {
      dialog.style.left =
        event.clientX + offsetx - dialog.offsetWidth / 2 + "px";
  ...
  LGraphCanvas.prototype.prompt = function (
    ...
    input.addEventListener("blur", (ev) => {
      if (ev.relatedTarget == input || ev.relatedTarget == button) return;
      dialog.close();
    });
  ...
  LGraphCanvas.prototype.showSearchBox = function (event, options) {
    ...
    function refreshHelper() {
      ...
      function addResult(type, className) {
        if (!LiteGraph.registered_node_types[type].showInContext) return;
